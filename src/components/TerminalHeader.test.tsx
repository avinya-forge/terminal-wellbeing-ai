...
describe('TerminalHeader', () => {
  ...
  it('labels the service as "Service Connected" when modelLoaded is true', () => {
    render(<TerminalHeader modelLoaded={true} />);
    // The outer ping ring <span> carries the title attribute
    expect(document.querySelector('[title="Service Connected"]')).toBeInTheDocument();
  });

  it('labels the service as "Loading…" when not loaded', () => {
    render(<TerminalHeader modelLoaded={false} />);
    expect(document.querySelector('[title="Loading…"]')).toBeInTheDocument();
  });
